globals [ max-sheep farmerA-part3 farmerB-part3 ]  ; don't let the sheep population grow too large

; Sheep and wolves are both breeds of turtles
breed [ sheep a-sheep ]  ; sheep is its own plural, so we use "a-sheep" as the singular
breed [ sheepB b-sheep ]
breed [ wolves wolf ]
...
to setup
  ...
  ask patches with [ (pycor > 28 and pycor < 32) or (pycor > -12 and pycor < -8)] [
    set pcolor black
  ]

  ask patches with [ (pycor > 8 and pycor < 12) or (pycor > -32 and pycor < -28)] [
    set pcolor blue
  ]

 set farmerA-part3  farmerA-number-sheep - farmerA-part1
 set farmerB-part3  farmerB-number-sheep - farmerB-part2

  create-sheep farmerA-part1 ;
  [
    set shape  "sheep"
    set color red
    ...
    setxy random-xcor random 18 + 11
  ]

  create-sheepB farmerB-part2 ;
  [
    set shape  "sheep"
    set color white
    set size 1.5  ; easier to see
    set label-color blue - 2
    set energy random (2 * sheep-gain-from-food)
    setxy random-xcor random 18 - 29
  ]

  create-sheep farmerA-part3 ;
  [
    set shape  "sheep"
    set color red
    set size 1.5  ; easier to see
    set label-color blue - 2
    set energy random (2 * sheep-gain-from-food)
    setxy random-xcor random 18 - 9
  ]

  create-sheepB farmerB-part3 ;
  [
    set shape  "sheep"
    set color white
    set size 1.5  ; easier to see
...
to go
  ...
  ask sheep [
    ...
    harvestA ; harvest sheep and reduce stock
  ]
  ...
  ask sheepB [
    ...
    reproduce-sheep  ; sheep reproduce at a random rate governed by a slider
    harvestB ; harvest sheep and reduce stock
  ]
...
to harvestA ; harvest procedure (i.e. sheepA harvest procedure)
  ; when grass increasing rate equals or below zero, harvest sheep
  if sheep-gain-from-food * ( count sheep ) >= ( ( count grass ) / 4 ) * 0.33 * ( 1 + grass-stockA-safe ) ;; but there probably could be starving sheep
    and (count sheep) * ( 1 - sheepA-harvest * 0.01 ) > sheepA-expand * farmerA-number-sheep  [
    show (count sheep) * sheepA-harvest * 0.01 show "sheepA sold out"
    ask n-of ( (count sheep) * sheepA-harvest * 0.01 ) sheep [die]
  ]
end

to harvestB ; harvest procedure (i.e. sheepB harvest procedure)
  ; when grass increasing rate equals or below zero, harvest sheep
  if sheep-gain-from-food * ( count sheepB ) >= ( ( count grass ) / 4 ) * 0.33 * ( 1 + grass-stockB-safe ) ;; but there probably could be starving sheep
    and (count sheepB) * ( 1 - sheepB-harvest * 0.01 ) > sheepB-expand * farmerB-number-sheep  [
    show (count sheepB) * sheepB-harvest * 0.01 show "sheepB sold out"
    ask n-of ( (count sheepB) * sheepB-harvest * 0.01 ) sheepB [die]
  ]
end
...
"" ""
PENS
"sheepA" 1.0 0 -612749 true "" "plot count sheep"
"sheepB" 1.0 0 -16449023 true "" "plot count sheepB"
