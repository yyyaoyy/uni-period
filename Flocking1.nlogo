globals[birds-speed-change birds-speed hawks-speed hawks-focus birds-distance birds-speed-flockmate]
patches-own[]
breed[hawks hawk]
breed[birds bird]

hawks-own[individual-speed]
...
to setup
  ...
  set birds-speed 0.7
  set hawks-speed 0.7
  set hawks-focus 0.85
  set birds-distance 2
  set birds-speed-change 0.01
  set birds-speed-flockmate 50
  ask patches[]

  create-hawks hawkspopulation
  [ set color red
    setxy random-xcor random-ycor
    set size 2  ;;Being bigger than the flock makes it easier to see.
    set individual-speed hawks-speed
  ]

  create-birds birdspopulation
    [ set color yellow - 2 + random 7  ;; random shades look nice
      set size 1.5  ;; easier to see
      setxy random-xcor random-ycor
      set flockmates no-turtles
      set individual-speed (birds-speed + random-float birds-speed-change)
  ]
  reset-ticks
end

to go
  bird-flock-avoid birds hawks birds-vision birds-avoid-minturn birds-avoid-maxturn birds-noise-movement birds-flock
  hunt hawks birds hawks-vision hawks-focus hawks-noise-movement
  tick
end

;;; MOVING
 ;;Walking around with prey around

to hunt [predator-breed prey-breed predator-vision predator-focus predator-noise-movement]
  ask predator-breed
  [
    let preys-around (prey-breed in-cone predator-vision 270)
    if (any? preys-around)
    [wander-around 10 predator-noise-movement]
    forward individual-speed
  ]
end

  ;;Birds runaway for avoiding or try to flock

to bird-flock-avoid [prey-breed predator-breed prey-vision  prey-avoid-minturn prey-avoid-maxturn prey-noise-movement flock-not]
  ask prey-breed
  [
    ifelse(any? predator-breed in-cone prey-vision 180)
    [runaway-noise predator-breed prey-avoid-minturn prey-avoid-maxturn prey-noise-movement]
    [flock-wander flock-not prey-breed prey-vision prey-noise-movement]
    forward individual-speed
  ]
end

to flock-wander [flock-not prey-breed prey-vision  prey-angle-noise]
  set flockmates other prey-breed in-cone prey-vision 180
ifelse (flock-not and (any? flockmates))
  [flock birds birds-vision  180 birds-speed birds-speed-flockmate birds-noise-movement]
  [wander-around 10 prey-angle-noise]
end

;;; BIRDS AVOIDING
 ;; If there is a hawk, the bird will steer in the minimum to maximum probability range

to runaway [avoid-breed min-turn max-turn]
  set heading (towards min-one-of avoid-breed [distance myself]) + (min-turn + random (max-turn - min-turn))
end

;;The bird escapes the hawk with some noise
to runaway-noise [predator-breed prey-avoid-minturn prey-avoid-maxturn prey-noise-movement]
  runaway predator-breed  prey-avoid-minturn prey-avoid-maxturn
  right random-angle prey-noise-movement
end

;;Go around
to wander-around [angle-wander angle-noise]
  right (random-angle angle-wander) + (random-angle angle-noise)
end

;;; BIRDS FLOCKING
  ;;Required functions for flocking (align, cohere and separate)

to flock [flocking-breed radius angle base-speed speed-flockmate noise-movement]
  set nearest-neighbor min-one-of flockmates [distance myself]
  ifelse (distance nearest-neighbor < birds-distance)[separate][align cohere]
  right random-angle noise-movement
  set individual-speed  base-speed + (speed-flockmate * count flockmates)
end

;;Make a turn of the maximum allowable degree
to turn-limit [desired-turn max-turn]
  ifelse (abs desired-turn > max-turn)
  [ifelse (desired-turn > 0)[right max-turn][left max-turn]]
  [ifelse (desired-turn > 0)[right abs desired-turn][left abs desired-turn]]
end

;;; SEPARATE
 ;;Stay away from the flocks that are too close

to separate  ;; turtle procedure
  let neighbor-heading ([heading] of nearest-neighbor)
  let difference-headings (subtract-headings heading neighbor-heading)
  turn-limit difference-headings max-separate-turn
end

;;; ALIGN
 ;;Turn the turtle in the average direction of its flockmates

to align  ;; turtle procedure
  let average-heading average-flockmate-heading
  let difference-headings (subtract-headings average-heading heading)
  turn-limit  difference-headings max-align-turn
end

to-report average-flockmate-heading  ;; turtle procedure
  ;;Use trigonometry: dx is simply the sine of the turtle's heading, and dy is simply the cosine.
...
;;; COHERE
 ;;Towards the average position of flockmates

to cohere  ;; turtle procedure
  let average-heading-towards average-heading-towards-flockmates
  let difference-headings (subtract-headings average-heading-towards heading)
  turn-limit difference-headings max-cohere-turn
end

to-report average-heading-towards-flockmates  ;; turtle procedure
  ;;Use 180 to head from me to the other turtle.
...
to-report random-angle[rangeS]
  ;;Returns an angle in the range (-range, range)
